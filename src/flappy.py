...
MAX_COUNT = 0 #count max scores reached to end training
...
def main():
    global DISCOUNT, MAX_THS, LR, HITMASKS, SCREEN, FPSCLOCK, FPS, bot, MODE, SCORES , EPISODE, MAX_SCORE, RESUME_ONCRASH
    ...
    parser.add_argument("--learning_rate", type=float, default=0.5, help="learning rate, default = 0.5")
    parser.add_argument("--ths", type=int, default=5, help="threshold for number of max scores reached, default = 5")
    parser.add_argument("--discount", type=float, default=1.0, help="Q function discount, default = 1")
    args = parser.parse_args()

    MAX_THS = args.ths #threshold for number of max scores reached
    FPS = args.fps
    ...
    # Initialize the bot
    bot = Bot(lr=LR, discount=DISCOUNT) 
...
def updateQtable(score):
    ...
    if MODE in [Mode.TRAIN, Mode.TRAIN_NOUI, Mode.TRAIN_REPLAY]:
        ...
        global MAX_COUNT
        if score >= MAX_SCORE and SCORES[-1] >= MAX_SCORE:
            MAX_COUNT+=1
            print(f'max score reached, +1: {MAX_COUNT}')
        else: MAX_COUNT = 0

        if MAX_COUNT >= MAX_THS:
...
def mainGame(movementInfo):
    ...
    while True:
       ...
       # check for crash here
        ...
        if MODE in [Mode.TRAIN_NOUI, Mode.TRAIN, Mode.TRAIN_REPLAY] and printHighScore and score != 0 and score % MAX_SCORE==0:
...
def showPerformance():
    ...
    plt.scatter(range(1, num+1), SCORES, c='red', s=3)
    plt.plot(range(1, num+1), average, 'b')
    plt.xlim((1,num))
    plt.ylim((0,int(max(SCORES)*1.1)))

    plt.title("Score distribution")
    plt.text(bot.gameCNT,max(SCORES), 
            f'lr={LR}, discount={DISCOUNT}', 
...
def getRandomPipe():
    ...
    PIPEGAPSIZE = random.randint(80,95)
